...
class Desisins : MainAPI() { // all providers must be an instance of MainAPI
    ...
    companion object {
        private val images = mutableMapOf<String, String>() // page url, image url
        private fun setImage(pageUrl: String, imageUrl: String) {
            images[pageUrl] = imageUrl
        }
    }
    ...
    override suspend fun loadLinks(
        ...
        val iframe = app.get(data).document.select(".responsive-iframe").attr("src")
        val resp = app.get(iframe).document
        val script =
            resp.select("script").find { it.data().contains("jwplayer(\"vplayer\").setup(") }
                ?.data()?.replace("file:", "\"file\":")
                ?.replace("label:", "\"label\":") ?: return false
        val src = Regex("(?<=sources: ).*(?=,)").find(script)?.value ?: return false
