...
class DirtyShip(private val plugin: DirtyShipPlugin) : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val galeriResimleri =
            document.select("div.col-md-7 div#album img").mapNotNull { it.attr("src") }
        ...
        val contentData = document.select("div.content-data.clearfix")
        val description = contentData.select(".the_description").first()?.select("strong")?.text()
        val actors = contentData.select("ul.post_performers").map { actor ->
            val actorIsim = actor.selectFirst("a")?.attr("title").toString()
            val actorPoster = actor.selectFirst("img")?.attr("src")
